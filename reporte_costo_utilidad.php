...
$sql_producto = _query("SELECT SUM(fd.cantidad) as cant, FORMAT(SUM(fd.precio_venta*fd.cantidad/pre.unidad),2) as precio, SUM(pre.costo*fd.cantidad/pre.unidad) as cost, p.descripcion, p.barcode, fd.id_prod_serv as idfd, p.id_producto as idp
  ...
  WHERE p.id_producto = fd.id_prod_serv AND pre.id_pp = fd.id_presentacion AND f.id_factura=fd.id_factura AND f.tipo_documento!='DEV' AND f.tipo_documento!='NC' AND f.anulada=0 AND f.credito=0 AND fd.fecha = '$fini' GROUP BY fd.id_prod_serv");
