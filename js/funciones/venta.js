...
$(document).ready(function() {
  ...
  $(document).on('keydown', '#num_ref', function(event) {
    ...
    if (event.keyCode == 13) {
      ...
      $("#borrar_preven").attr('disabled', true);
      $("#preventa").attr('disabled', true);
      if ($(this).val() != "") {
        a = $('#vendedor');
        b = $('#id_cliente');
        c = $('#tipo_impresion');
...
function senddata() {
  ...
  if (sel_vendedor == 1) {
    ...
    $.ajax({
      ...
      success: function(datax) {
        if (datax.typeinfo == "Success") {
          ...
          display_notify(datax.typeinfo, datax.msg);
          setTimeout(function() {
            $("#pago_left").removeClass('col-md-12');
            $("#pago_left").addClass('col-md-9');
            $("#pago_right").show('slow/100/fast', function() {
            });
          },100);
    ...
    .fail(function() {
      console.log("error");
      display_notify("Error","Tiempo excedido verifique si se guardo la factura")
    });
  } else {
    display_notify('Warning', msg);
    $("#submit1").removeAttr('disabled');
...
function imprimev() {
  ...
  var dataString = 'process=' + print + '&numero_doc=' + numero_doc 
    ...
    + '&numero_factura_consumidor=' + numero_factura_consumidor
    + '&fecha_fact=' + fecha_fact + '&duiCliente='+duiCliente;
